...
export default async function Page({ params }: { params: { uid: string } }) {
    ...
    for (let i = 0; i < characters.length; ++i) {
        ...
        charList.push(<div className="group rounded-md border backdrop-blur-xl bg-white/25 px-3 cursor-pointer z-10 mb-2" >{buildCharacterCard(characters[i], true, false)}</div>)
    }
    
    let content = <div className={"pl-1 w-full h-screen flex flex-col place-content-center items-center"}>
                            <div className={"columns-1 xl:columns-5 lg:columns-4 md:columns-3 sm:columns-2 rounded-md border backdrop-blur-xl bg-white/25 p-2 w-3/4 z-10 border-slate-400"}>
                                {charList}
                            </div>
                    </div>
