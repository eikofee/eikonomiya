...
export default function TopTabSwitcher({currentMode, currentModeCallback} : {currentMode: ETabMode, currentModeCallback: (e: ETabMode) => void}) {
    ...
    <div className={"flex flex-row items-center h-full rounded-md"}>
        <div className="cursor-pointer w-8 h-8 px-1" onClick={() => currentModeCallback(ETabMode.ARTIFACTS)}>
            <Icon n="fleur" useTooltip={false} customStyle={currentMode == ETabMode.ARTIFACTS ? "fill-black" : "fill-black/30"}/>
        </div>
        <div className="cursor-pointer w-8 h-8 px-1" onClick={() => currentModeCallback(ETabMode.TALENT_VALUES)}>
