...
export default function WeaponCard({equip, rule} : {equip: IWeapon, rule: ICharacterRule}) {
    ...
    if (equip.subStat != undefined) {
        ...
        <div className="w-full flex flex-row items-center">
                ...
                <div className={"text-right grow"}>
                    {isPercentage(equip.subStat.name) ? (equip.subStat.value! * 100).toFixed(1): equip.subStat.value}{isPercentage(equip.subStat.name) ? "%" : ""}
                </div>
            </div>
        </div>
    </li>
    ...
    <div className="aspect-square grad-5star basis-1/5 flex items-center justify-center rounded-t-md">
        <InfoDiv child={<img src={equip.name} className="max-w-full max-h-full"/>} info={<p>{equip.name}</p>} infoClassname="transition duration-300 z-10 absolute bg-gray-800 text-white font-normal text-sm rounded p-2 whitespace-nowrap top-full left-1/2 transform -translate-x-1/2" />
    </div>
